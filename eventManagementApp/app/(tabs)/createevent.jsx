...
const CreateEvent = () => {
  ...
  const createEventHandler=async()=>{
    ...
    console.log(dataObj);
    try {
      const res=await fetch(`${baseAddress}/create-event/${userId}`,{
        method:"POST",
        headers:{
  ...
  return (
    ...
    <View >
      ...
      <TextInput placeholder='event name..' style={{fontSize:15,color:'gray',backgroundColor:'#d9d9d9',padding:16,borderRadius:10}} onChangeText={(text)=>setEventName(text)} />
        ...
        <TextInput placeholder='description..' style={{fontSize:15,color:'gray',backgroundColor:'#d9d9d9',padding:16,borderRadius:10}} onChangeText={(text)=>setDescription(text)}/>
            ...
            <ScrollView horizontal style={{marginVertical:10}}>
            {
              ...
              <Pressable key={image._id} onPress={()=>setImageUrl(image.url)}  >{image.url==imageUrl && <View style={{position:'absolute',backgroundColor:'black',height:'100%',width:'100%',borderRadius:20,zIndex:100,opacity:0.4,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:'column'}}><AntDesign name="checkcircle" size={40} color="white" /><Text style={{color:'white',fontWeight:'bold',fontSize:30}}>Selected</Text></View>}<Image source={{uri:image.urlforShowing}} style={{height:200,width:260,borderRadius:20,backgroundColor:"black"}} /></Pressable>)
            }
             </ScrollView>
            </View>
       <View>
      ...
      <Pressable style={{backgroundColor:'#7186f1',padding:16,borderRadius:10,display:'flex',flexDirection:"row",justifyContent:"center",marginTop:20,marginBottom:70}} onPress={createEventHandler} > 
